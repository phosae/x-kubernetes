...
ifeq (, $(shell which kind))
	GOBIN=/usr/local/bin/ go install sigs.k8s.io/kind@v0.27.0
endif
...
ifeq (, $(shell which ko))
	GOBIN=/usr/local/bin/ go install github.com/google/ko@v0.17.1
endif

kustomize: # find or download kustomize if necessary
ifeq (, $(shell which kustomize))
	GOBIN=/usr/local/bin/ go install -ldflags="-X 'sigs.k8s.io/kustomize/api/provenance.version=v5.6.0'" sigs.k8s.io/kustomize/kustomize/v5@v5.6.0
endif